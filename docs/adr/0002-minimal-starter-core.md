...
- Start with a broad prefixed class set such as `o-stack`, `o-grid`, `c-button`, `c-card`, `c-prose`, `u-visually-hidden`, and state classes.
- Start with short unprefixed builder-friendly classes and expand only when a project proves the need.

## Consequences

The default setup stays easier to apply in builder class interfaces and avoids becoming shipped standalone CSS by accident. Optional recipes and expansion naming can still be added per project when they create clear layout, component, accessibility, or content value.
